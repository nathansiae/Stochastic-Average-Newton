...
if __name__ == '__main__':
    ...
    dict_algo_norm, dict_algo_time, dict_algo_fval = {}, {}, {}

    for algo_name in config.algorithms2run.keys():
        algo = config.algorithms2run[algo_name]
        np.random.seed(0)  # random seed to reproduce the experiments
        kwargs = {"loss": criterion, "data": X, "label": y, "lr": config.lr, "reg": reg,  "epoch": epochs,
                  "x_0": x_0.copy(), "regularizer": penalty, "tol": config.tol}
        if "san" in algo_name.lower():
            kwargs["dist"] = dist
        if "snm" in algo_name.lower():
        ...
        grad_iter, grad_time, fval_iter = utils.run_algorithm(
            algo_name=algo_name.upper(), algo=algo, algo_kwargs=kwargs, n_repeat=n_rounds)
